...
def select_cve_ai(context:Context, connection: connection, cve_id: str):
...
def insert_sql(context:Context, connection: connection, table_name: str, columns, values):
	cursor: psycopg2.extensions.cursor | None = None
	sql_query = "(not created)"
	try:
		cursor = connection.cursor()
...
# dbの接続を閉じる関数
def close_connection(context:Context, connection: connection):
	try:
		if connection:
			connection.close()
	except Exception as e:
